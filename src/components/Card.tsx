...
export default function Card({ href = '', frontmatter, secHeading = true }: Props) {
  ...
  return (
    <li
      ...
      style={{ borderColor: 'var(--color-text-base)' }}
    >
      <a
        href={href}
        ...
        style={{ color: 'var(--link-color-primary)' }}
      >
        {secHeading ? <h2 {...headerProps}>{title}</h2> : <h3 {...headerProps}>{title}</h3>}
      </a>
      <Datetime pubDatetime={pubDatetime} modDatetime={modDatetime} />
      <p style={{ color: 'var(--blog-color-secondary)' }}>{description}</p>
      <div className="flex flex-wrap">
        {tags?.map((tag) => (
          <a
            href={`/tags/${tag.toLowerCase()}/`}
            key={tag}
            style={{ color: 'var(--link-color-secondary)' }}
