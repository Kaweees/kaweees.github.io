...
import { siteConfig, user } from "$lib/config";
...
const getAuthors = () => {
  return Object.entries(
    import.meta.glob("/content/authors/**/*.md", { eager: true })
  );
};
...
const getMetadata = (entryType, filepath, entry) => {
  return {
    ...
    author:
      entryType === "posts" && !siteConfig.multiuser
        ? user.name
...
export const getEntries = (entryType) => {
  if (!siteConfig.multiuser && entryType === "authors") return [user];
